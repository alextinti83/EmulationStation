...
InputConfig::InputConfig(int deviceId, const std::string& deviceName, const std::string& deviceGUID, 
	...
	mDeviceIndex(deviceIndex), mDeviceNbAxes(deviceNbAxes)
{
}
...
bool InputConfig::getInputByName(const std::string& name, Input* result) const
{
	auto it = mNameMap.find(toLower(name));
	if(it != mNameMap.end())
	{
...
bool InputConfig::isMappedTo(const std::string& name, Input input) const
{
	Input comp;
